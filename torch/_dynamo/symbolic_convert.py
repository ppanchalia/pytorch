...
class InstructionTranslatorBase(
    ...
    def prune_dead_locals(self):
        ...
        #    initialized properly by its creater, i.e., the tuple of cells will
        ...
        #    the tuple of cells _after_ emitting the `MAKE_FUNCTION` bytecode,
        #    via `STORE_DEREF`; these `STORE_DEREF` are generated partly based
        ...
        # again. So here we do some conservative pruning, by tracing from all
        # living `symbolic_locals` that are _not_ cells; for any reachable cell,
        # it must be kept alive.
        #
        # TODO there are extra complexities due to side-effects (e.g., the
        # nested function leaking out into backward hook or globals). We could
        # probably improve the variable tracing here to include the relevant
        # variables in `output.side_effects`.
        if self.output.side_effects.is_empty():
            ...
            def visit(var: VariableTracker):
                ...
                # no more variable to reach after `LazyVariableTracker` anyways.
                if isinstance(var, LazyVariableTracker):
                    return

                # NOTE: we don't handle `InlinedClosureVariable` because the
                # `STORE_DEREF` for these cells would've been generated by the
                # parent function, where these `InlinedClosureVariable` are
                # represented as `ClosureVariable`.
